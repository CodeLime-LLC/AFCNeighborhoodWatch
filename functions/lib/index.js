...
const fetchTransfers_1 = require("./fetchTransfers");
...
async function readSourceStatus() {
    ...
    const salesMax = data.sourceMaxSaleDate?.toDate() ??
        null;
    const transferMax = data.sourceMaxTransferDate?.toDate() ??
        null;
    const newestSaleDate = salesMax && transferMax
...
async function runPipeline(trigger) {
    ...
    const salesRecords = (0, fetchSales_1.toSaleRecords)(filtered, years[0]);
    ...
    let sourceMaxTransferDate = null;
    try {
        const inventory = await (0, fetchTransfers_1.fetchInventoryCsv)(config.jurisdictionCode);
        const { rows: transfers, newestTransfer } = (0, fetchTransfers_1.parseTransfers)(inventory.text, cutoffDate);
        sourceMaxTransferDate = newestTransfer;
        inventoryRecords = (0, fetchTransfers_1.toSaleRecordsFromTransfers)(transfers, years[0], "ANKENY");
        console.log(`Inventory: newest transfer ${newestTransfer?.toISOString().slice(0, 10) ?? "none"}, ` +
    ...
    console.log(`Freshness: sales export newest sale ` +
    ...
    for (const doc of existingSnap.docs) {
        const key = doc.data().sourceKey;
        if (key) {
            existing.set(key, {
                id: doc.id,
                source: doc.data().source ?? "sales",
            });
        }
    }
    ...
    const upgrades = records.filter((r) => r.source === "sales" && existing.get(r.sourceKey)?.source === "inventory");
