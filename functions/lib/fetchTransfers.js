...
exports.looksLikeEntity = looksLikeEntity;
exports.isOwnerOccupied = isOwnerOccupied;
exports.fetchInventoryCsv = fetchInventoryCsv;
exports.parseTransfers = parseTransfers;
exports.toSaleRecordsFromTransfers = toSaleRecordsFromTransfers;
const sync_1 = require("csv-parse/sync");
const fetchSales_1 = require("./fetchSales");
const INVENTORY_BASE_URL = "https://web.assess.co.polk.ia.us/info/web/exports/res/inven/juris";
/**
...
function looksLikeEntity(name) {
    ...
    if (ENTITY_PHRASES.some((p) => upper.includes(p)))
        return true;
    return upper
        .split(/[^A-Z0-9]+/)
        .filter(Boolean)
        .some((w) => ENTITY_WORDS.has(w) || ENTITY_STEMS.some((st) => w.startsWith(st)));
}
/**
 ...
 */
function isOwnerOccupied(row) {
    const same = (a, b) => (a ?? "").trim().toUpperCase() === (b ?? "").trim().toUpperCase();
    return (same(row.house, row.mail_house) &&
 ...
 */
async function fetchInventoryCsv(jurisdictionCode) {
    const url = `${INVENTORY_BASE_URL}/${jurisdictionCode}.csv`;
    const response = await fetch(url);
    if (!response.ok) {
        throw new Error(`Failed to fetch inventory CSV: ${response.status} from ${url}`);
    }
 ...
 */
function parseTransfers(csvText, cutoffDate) {
    let newestTransfer = null;
    const rows = (0, sync_1.parse)(csvText, {
        ...
        on_record: (row) => {
            const transferred = (0, fetchSales_1.parseSaleDate)(row.transfer_th1 ?? "");
            if (!transferred)
                return null;
            ...
            if (transferred < cutoffDate)
                return null;
            if (!RESIDENTIAL_OCCUPANCY.has(row.occupancy ?? ""))
                return null;
            if (!row.book_th1 || !row.pg_th1)
                return null;
            const owner = (row.title_holder1 ?? "").trim();
            if (!owner)
                return null;
            if (!isOwnerOccupied(row))
                return null;
            if (looksLikeEntity(owner))
                return null;
            return {
 ...
 */
function toSaleRecordsFromTransfers(rows, fetchYear, defaultCity) {
    const results = [];
    for (const row of rows) {
        const transferred = (0, fetchSales_1.parseSaleDate)(row.transferDate);
        if (!transferred)
            continue;
        results.push({
            address: row.address,
...
//# sourceMappingURL=fetchTransfers.js.map